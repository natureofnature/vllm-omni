...
def _make_manager(
    ...
    with (
        patch.object(OmniKVTransferManager, "_get_local_rank", return_value=local_rank),
        patch.object(OmniKVTransferManager, "_get_tp_world_size", return_value=max(from_tp, to_tp)),
    ):
...
class TestConnectorKeyFormat:
    def test_key_format_matches_pr2677(self):
        key = OmniKVTransferManager.get_kv_connector_key("req-1", "stage0", 0, 1, 2)
        assert key == "req-1_stage0_0_1_2"

    def test_key_fields_are_positional(self):
        key = OmniKVTransferManager.get_kv_connector_key("r", "s", 5, 3, 7)
        parts = key.split("_")
...
class TestRankMapping:
    """Verify _get_kv_target_ranks and _get_kv_source_ranks for various TP configs."""

    def test_homogeneous_tp2_rank0(self):
        mgr = _make_manager(from_tp=2, to_tp=2, local_rank=0)
        assert mgr._get_kv_target_ranks() == [0]
        assert mgr._get_kv_source_ranks() == [0]

    def test_homogeneous_tp2_rank1(self):
        mgr = _make_manager(from_tp=2, to_tp=2, local_rank=1)
        assert mgr._get_kv_target_ranks() == [1]
        assert mgr._get_kv_source_ranks() == [1]

    def test_homogeneous_tp4_rank3(self):
        mgr = _make_manager(from_tp=4, to_tp=4, local_rank=3)
        assert mgr._get_kv_target_ranks() == [3]
        assert mgr._get_kv_source_ranks() == [3]

    def test_sender_gt_receiver_tp4_to_tp2_rank0(self):
        """Receiver rank 0 should receive from sender rank 0 and 1."""
        mgr = _make_manager(from_tp=4, to_tp=2, local_rank=0)
        assert mgr._get_kv_source_ranks() == [0, 1]

    def test_sender_gt_receiver_tp4_to_tp2_rank1(self):
        """Receiver rank 1 should receive from sender rank 2 and 3."""
        mgr = _make_manager(from_tp=4, to_tp=2, local_rank=1)
        assert mgr._get_kv_source_ranks() == [2, 3]

    def test_sender_lt_receiver_tp2_to_tp4_rank0(self):
        """Sender rank 0 should send to receiver ranks 0 and 1."""
        mgr = _make_manager(from_tp=2, to_tp=4, local_rank=0, need_send=True, need_recv=False)
        assert mgr._get_kv_target_ranks() == [0, 1]

    def test_sender_lt_receiver_tp2_to_tp4_rank1(self):
        mgr = _make_manager(from_tp=2, to_tp=4, local_rank=1, need_send=True, need_recv=False)
        assert mgr._get_kv_target_ranks() == [2, 3]
    ...
    def test_invalid_topology_raises(self):
        mgr = _make_manager(from_tp=3, to_tp=2, local_rank=0)
        with pytest.raises(ValueError, match="divisible"):
            mgr._get_kv_source_ranks()
...
class TestBuildRankAwareRecvKeys:
    """Verify _build_rank_aware_recv_keys returns (key, from_rank) tuples."""

    def test_tp1_returns_legacy_key_with_none_rank(self):
        mgr = _make_manager(from_tp=1, to_tp=1, local_rank=0)
        pairs = mgr._build_rank_aware_recv_keys("req-1", "stage0", "stage1")
        assert len(pairs) == 1
        key, rank = pairs[0]
    ...
    def test_heterogeneous_tp4_to_tp2_rank0_gets_two_keys(self):
        """Receiver rank 0 with from_tp=4, to_tp=2 should get 2 keys."""
        mgr = _make_manager(from_tp=4, to_tp=2, local_rank=0)
        pairs = mgr._build_rank_aware_recv_keys("req-1", "stage0", "stage1")
        assert len(pairs) == 2
    ...
    def test_heterogeneous_tp2_to_tp4_rank2_gets_one_key(self):
        """Receiver rank 2 with from_tp=2, to_tp=4 should get 1 key from sender rank 1."""
        mgr = _make_manager(from_tp=2, to_tp=4, local_rank=2)
        pairs = mgr._build_rank_aware_recv_keys("req-1", "stage0", "stage1")
        assert len(pairs) == 1
        key, rank = pairs[0]
        assert rank == 1
...
class TestBuildRankAwareSendKeys:
    def test_tp1_returns_legacy_key(self):
        mgr = _make_manager(from_tp=1, to_tp=1, local_rank=0, need_send=True, need_recv=False)
        keys = mgr._build_rank_aware_send_keys("req-1", "stage0", "stage1")
        assert keys == ["omni_stage0_to_stage1_kv_cache_req-1"]

    def test_homogeneous_tp2_rank0(self):
        mgr = _make_manager(from_tp=2, to_tp=2, local_rank=0, need_send=True, need_recv=False)
        keys = mgr._build_rank_aware_send_keys("req-1", "stage0", "stage1")
...
class TestUpdateSenderInfoBase:
    ...
    def test_nested_sender_info_resolves_correctly(self):
        ...
        with (
            patch.object(OmniKVTransferManager, "_get_local_rank", return_value=0),
            patch.object(OmniKVTransferManager, "_get_tp_world_size", return_value=2),
        ):
            mgr = OmniKVTransferManager(config)
...
class TestReceiveConstructsMetadata:
    ...
    def test_heterogeneous_tp4_to_tp2_rank0_multiple_metadata(self):
        """Receiver rank 0 with from_tp=4, to_tp=2 should call get() with
...
class TestMergeSliceHooks:
    def test_single_shard_passes_through(self):
        ...
        assert mgr._merge_received_rank_shards([payload]) == payload

    def test_default_merger_concats_head_dim(self):
        mgr = _make_manager(from_tp=4, to_tp=2, local_rank=0)
        p0 = _make_payload([0.0])
        p1 = _make_payload([1.0])
        result = mgr._merge_received_rank_shards([p0, p1])
    ...
    def test_custom_merger_hook_called(self):
        mgr = _make_manager(from_tp=4, to_tp=2, local_rank=0)
        merged = {"merged": True}
        mgr.kv_payload_merger = lambda payloads: merged
        assert mgr._merge_received_rank_shards([{}, {}]) == merged

    def test_slicer_hook_called(self):
        mgr = _make_manager(from_tp=2, to_tp=4, local_rank=0)
        sliced = {"sliced": True}
        mgr.kv_payload_slicer = lambda payload: sliced
        assert mgr._slice_received_rank_shard({"full": True}) == sliced
    ...
    def test_default_slicer_extracts_rank_local_heads(self):
        ...
        result = mgr._slice_received_rank_shard(payload)
        key_cache = result["layer_blocks"]["key_cache"][0]
        value_cache = result["layer_blocks"]["value_cache"][0]
...
class TestAutoDetectTP:
    def test_auto_detect_when_config_defaults(self):
        ...
        with (
            patch.object(OmniKVTransferManager, "_get_local_rank", return_value=0),
            patch.object(OmniKVTransferManager, "_get_tp_world_size", return_value=4),
        ):
            mgr = OmniKVTransferManager(config)
        assert mgr._from_tp == 4
        assert mgr._to_tp == 4
    ...
    def test_explicit_tp_overrides_auto_detect(self):
        ...
        with (
            patch.object(OmniKVTransferManager, "_get_local_rank", return_value=0),
            patch.object(OmniKVTransferManager, "_get_tp_world_size", return_value=8),
        ):
            mgr = OmniKVTransferManager(config)
        assert mgr._from_tp == 2
        assert mgr._to_tp == 4
