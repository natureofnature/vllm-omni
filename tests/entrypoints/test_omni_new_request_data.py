...
def test_omni_new_request_data_copies_payloads():
    ...
    assert data.initial_model_buffer is initial_model_buffer
...
def _build_wrapped_seed_buffer_case():
    scheduled = NewRequestData(
        req_id="req-5",
    ...
    return OmniNewRequestData.from_scheduled_request_data(scheduled, request)


def _build_from_request_seed_buffer_case():
    request = SimpleNamespace(
        request_id="req-6",
    ...
    return OmniNewRequestData.from_request(request, ([0],), prefill_token_ids=None)


@pytest.mark.parametrize(
    "builder",
    [_build_wrapped_seed_buffer_case, _build_from_request_seed_buffer_case],
    ids=["from_scheduled_request_data", "from_request"],
)
def test_omni_new_request_data_prefers_runtime_seed_buffer(builder):
    data = builder()
