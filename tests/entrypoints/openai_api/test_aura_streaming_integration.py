...
async def test_aura_websocket_streams_text_and_audio(monkeypatch):
    ...
    await handler._process_query_engine(
        ...
        frame_metadata=[
            {
                "frame_id": "aura-frame-1",
                "pts_ms": 100,
                "receiver_received_ts_ms": 1.0,
            }
        ],
    )
    ...
    assert "video.frames.consumed" in types
    ...
    assert consumed["request_id"] == "req-aura-audio"
    assert consumed["frame_ids"] == ["aura-frame-1"]
    assert consumed["latest_pts_ms"] == 100
    ...
    turn_events = [
        message for message in ws.sent if message.get("type", "").startswith(("response.", "user.transcript."))
    ]
    assert turn_events
