...
def test_code2wav_input_reshape_ignores_trailing_padding() -> None:
    ...
    assert codes.shape == (2, 16, 2)
    torch.testing.assert_close(codes[0, :, 0], torch.arange(16, dtype=torch.long))
    torch.testing.assert_close(codes[1], torch.arange(16, 48, dtype=torch.long).reshape(16, 2))
    assert not torch.any(codes == 777)
