    ...
    async def run_mailbox(self) -> None:
        ...
        while True:
            ...
            if event_type in {"input.cancel", "response.cancel", "barge_in", "output_audio_buffer.clear"}:
                await self.owner._handle_cancel_event(
                    event=event,
            ...
            if event_type == "turn.signal":
                await self.owner._handle_turn_signal(
                    event=event,
                    event_type=event_type,
                    session=self.session,
                    emit_event=self.emit_event,
            ...
            if event_type == "input.text.append":
                await self.owner._handle_text_append(
                    event=event,
                    session=self.session,
                    emit_event=self.emit_event,
            ...
            if event_type == "input_audio_buffer.append":
                await self.owner._handle_audio_append(
                    event=event,
                    event_type=event_type,
                    session=self.session,
                    actor=self.actor,
            ...
            if event_type in {"input.commit", "input_audio_buffer.commit", "response.create"}:
                await self.owner._handle_commit_event(
                    event=event,
                    event_type=event_type,
                    session=self.session,
                    actor=self.actor,
