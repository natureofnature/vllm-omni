...
class OmniConnectorModelRunnerMixin:
    ...
    def init_omni_connectors(
        ...
        # Full stage payloads stay here after connector receive.
        # Downstream preprocess / execute reads from this cache directly.
    ...
    def cleanup_finished_request(self, req_id: str) -> None:
        ...
        with self._lock:
            if self._pending_save_counts.get(send_req_id, 0):
                ...
                self._request_payload.pop(send_req_id, None)
    ...
    def mark_kv_transfer(self, req_id: str, seq_len: int, block_ids: list[int]) -> None:
    ...
    def get_omni_connector_output(self) -> OmniConnectorOutput:
        ...
        with self._lock:
            ...
            for req_id in newly_finished:
                ...
                self._request_payload.pop(ext_req_id, None)
        ...
        try:
            ...
            with self._lock:
                ...
                if cleanup_req_id is not None:
                    ...
                    self._request_payload.pop(cleanup_req_id, None)
    ...
    def _accumulate_payload(self, req_id: str, payload_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if req_id not in self._request_payload:
            self._request_payload[req_id] = dict(payload_data)
            return self._request_payload[req_id]
        ...
        self._request_payload[req_id] = merged
        return merged
