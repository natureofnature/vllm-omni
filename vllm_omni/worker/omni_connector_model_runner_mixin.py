...
class OmniConnectorModelRunnerMixin:
    ...
    def get_omni_connector_output(self) -> OmniConnectorOutput:
        ...
        with self._lock:
            ...
            # Reset accumulated payloads so that next delivery cycle
            # starts fresh — prevents re-delivering already-consumed
            # embeds when concatenation is used instead of override.
            for internal_id in chunk_data:
                ext_id = self._request_ids_mapping.get(internal_id, internal_id)
                self._request_payload.pop(ext_id, None)
    ...
    def _accumulate_payload(self, req_id: str, payload_data: dict[str, Any]) -> dict[str, Any]:
        ...
        # thinker_decode_embeddings must be concatenated, not replaced:
        # each chunk carries a unique embed that the model caches and
        # consumes by position.  Replacing loses intermediate embeds
        # when multiple chunks arrive between delivery cycles.
        concat_always = {"thinker_decode_embeddings"}
        for key, value in payload_data.items():
            if key == "finished":
                continue
            if key in override_keys and key not in concat_always:
