...
from vllm_omni.entrypoints.stage_utils import shm_write_bytes, shm_read_bytes
    ...
    def get(self, from_stage: str, to_stage: str, request_id: str, metadata: Optional[dict[str, Any]] = None) -> Optional[tuple[Any, int]]:
        ...
        try:
            ...
            if "shm" in metadata:
                ...
                logger.error(f"Unknown metadata format in SharedMemoryConnector for req {request_id}: {list(metadata.keys())}")
                return None
