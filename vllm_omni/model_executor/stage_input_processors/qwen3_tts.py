...
def talker2code2wav_full_payload(
    ...
    ref_code_len = pooling_output.get("ref_code_len")
    ...
    codec_codes, ref_code_len = _flatten_qwen3_tts_codes(
        audio_codes,
        ref_code if isinstance(ref_code, torch.Tensor) else None,
    )
    if not codec_codes:
        return None
    payload = {
...
def talker2code2wav_async_chunk(
    ...
    # Per-request override takes priority over config default and dynamic IC.
    ...
    if not per_request_override and initial_chunk_size <= 0:
    ...
    info: dict[str, Any] = {
        ...
        "code_num_quantizers": num_quantizers,
        "finished": finished,
    }
