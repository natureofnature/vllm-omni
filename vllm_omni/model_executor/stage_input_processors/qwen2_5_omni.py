...
def thinker2talker_batch(
    ...
    logger.debug(
        "thinker2talker_batch: prompt_len=%s thinker_result.shape=%s prompt_embeds.shape=%s output_token_ids_len=%s",
        prompt_len,
...
def talker2code2wav_batch(
    ...
    logger.debug(
        "talker2code2wav_batch: original_len=%s filtered_len=%s first_10=%s last_10=%s",
        len(output_ids),
        len(filtered_ids),
        filtered_ids[:10],
        filtered_ids[-10:],
