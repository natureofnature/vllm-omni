...
def _reshape_code2wav_input_ids(
    ...
    input_ids_flatten = input_ids_flatten[: sum(seq_token_counts)]
...
class Qwen3OmniMoeForConditionalGeneration(
        ...
        **kwargs: object,
        ...
        # ========== Stage 3: Code2Wav ==========
            ...
            if seq_token_counts is not None:
                num_seq_tokens = sum(seq_token_counts)
                if num_seq_tokens > num_input_ids:
                    logger.warning_once(
                        "Code2Wav received scheduler token counts that exceed input_ids; "
    ...
    def _get_talker_assistant_parts(
        ...
        assistant_prefix_hidden = assistant_hidden[:3]
        if assistant_prefix_hidden.shape[0] < 3:
            assistant_prefix_hidden = torch.cat(
                (
                    assistant_prefix_hidden,
                    torch.zeros(
                        (3 - assistant_prefix_hidden.shape[0], assistant_hidden.shape[1]),
